...
class eFedGauss(torch.nn.Module):
    def __init__(self, feature_dim, num_classes, kappa_n, num_sigma, kappa_join, S_0, N_r, c_max, device):
        ...
        #Support tensors
        self.score = torch.empty((self.current_capacity,), dtype=torch.float32, device=device) #Initialize cluster scores
        self.num_pred = torch.empty((self.current_capacity,), dtype=torch.float32, device=device) #Initializs number of predictions
    ...
    def clustering(self, data, labels):
        ...
        for (z, label) in zip(data, labels):
            ...
            # Select all clusters for distance computation
            ...
            if self.evolving:
                with torch.no_grad():
                    ...
                    # Select clusters for evolution based on the label
    ...
    def forward(self, data):
        ...
        # Compute activation to the clusters for all samples
        ...
        # Get the cluster indices for the entire batch
