...
class FederalOps:
    ...
    def federated_merging(self):
        ...
        for label in range(0, self.parent.num_classes):
            ...
            # Select a subset of cluster centers instead of all, to reduce computational demand
            ...
            if self.parent.c > self.parent.c_max:
                #self.parent.removal_mech.remove_solo() #First remove cluster with one sample as those can not be transmited.
                self.parent.matching_clusters = torch.where(self.parent.cluster_labels[:self.parent.c][:, label])[0]
