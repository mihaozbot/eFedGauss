...
class RemovalMechanism:
    ...
    def update_cluster_scores(self, cluster_indices, label, correct=True):
        ...
        #Increase the score if classification is correct
        ...
        #Increase the number of prediction
    ...
    def remove_solo(self):
        with torch.no_grad():
            # Identify clusters with only one sample
            solo_clusters = [idx for idx, size in enumerate(self.parent.n[self.parent.matching_clusters]) if size == 1]
            solo_clusters.reverse() 

            # Remove the identified solo clusters
            for index in solo_clusters:
                self.remove_cluster(index)
    ...
    def remove_cluster(self, cluster_index):
        ...
        if cluster_index != last_active_index: #The target cluster is not the last cluster
            ...
            self.parent.mu.data[cluster_index] = self.parent.mu.data[last_active_index]
